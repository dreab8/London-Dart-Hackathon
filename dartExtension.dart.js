...
Parser.prototype._parseFuture = function(name) {
  ...
  packagesKeys.forEach$1((function (k) {
    ...
    packageValues.forEach$1((function (v) {
      if ($eq$(v.$index("name"), "AssertionError")) {
        print$($add$($add$($add$($add$("found ", v.$index("name")), " "), $this.baseUrl), v.$index("url")));
        var result = new Result(v.$index("name"), $add$($this.baseUrl, v.$index("url")));
        print$($add$("name ", result.name));
